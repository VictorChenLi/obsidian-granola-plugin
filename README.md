...
|---------|---------|-------------|
| Time range | Last 30 days | How far back to look for meetings. Preset ranges are discovered from the Granola MCP server (`this_week`, `last_week`, `last_30_days`). Picking **All time** requests every meeting via the server's `custom` range with a year-2000 floor. Picking **Custom range** reveals two date pickers so you can choose any start/end dates. |
